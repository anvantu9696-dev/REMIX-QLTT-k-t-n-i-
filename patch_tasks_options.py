...
option_block_multi = """                                          {item.input_type === 'OPTION' && (() => {
                                            let opts: string[] = [];
                                            try {
...
option_block_legacy = """                                    {item.input_type === 'OPTION' && (() => {
                                      let opts: string[] = [];
                                      try {
                                        opts = item.options_json ? JSON.parse(item.options_json) : [];
                                      } catch(e) {}
...
code = code.replace("{item.input_type === 'PASS_FAIL' && (", option_block_multi + "\n                                          {item.input_type === 'PASS_FAIL' && (", 1)
code = code.replace("{item.input_type === 'PASS_FAIL' && (", option_block_legacy + "\n                                    {item.input_type === 'PASS_FAIL' && (", 1)
